...
class RNN_path(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Conv1d(in_channels = 1,
                               ...
                               kernel_size = 1210,
                               stride = 12,
                               padding_mode = 'valid',).to(self.device)
        self.activation = nn.SELU(inplace = True).to(self.device)
        ...
        self.output_activation = nn.Sigmoid().to(self.device)
    ...
    def forward(self,x):
        ...
        rnn,_ = self.rnn(conv2.permute(0,2,1))
        output = self.output_activation(rnn)
        return torch.squeeze(output)

def train_loop(computer_vision_net,sound_net,loss_func,optimizers,dataloader,
               idx_epoch = 1,
               l1 = 0.,l2 = 0.01,
    ...
    for ii,(batch) in enumerate(dataloader):
        if ii + 1 <= len(dataloader):
            ...
            loss_CVN = loss_func(output_CVN,output_SN)
            selected_params = torch.cat([x.view(-1) for x in computer_vision_net.parameters()]) # L2 
